...
HRESULT Inventory::init(void)
{
	...
	for (int y = 0; y < INVENTORY_Y; y++) {
		for (int x = 0; x < INVENTORY_X; x++) {
			...
			m_Inventory[y][x]._rect = RectMake(m_rc.left + 20 + x * 33 + MAPMANAGER->getCamera().x, m_rc.top + 4 + y * 36 + MAPMANAGER->getCamera().y, 13, 13);
		}
	}
...
void Inventory::render(HDC hdc)
{
	if (invenOn) {
		...
		m_InventoryImg->render(hdc, m_rc.left + MAPMANAGER->getCamera().x, m_rc.top - 35 + MAPMANAGER->getCamera().y, 223, m_InvenY);
		m_SelectImg->render(hdc, Select_Rect.left + MAPMANAGER->getCamera().x, Select_Rect.top + MAPMANAGER->getCamera().y, 25, 25);
